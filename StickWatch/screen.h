...
void showSplashScreen(boolean isReset) {
  ...
  if (isReset) {
    ...
    delay(2000);
    u8g2.clearBuffer();
    u8g2.drawStr(10, 20, "M5WATCH/V1.76-r3");
    u8g2.sendBuffer();
...
void showWifiConnected() {
  u8g2.clearBuffer();
  u8g2.drawStr(10, 20, "CONNECTED[NTPro] :D");
  u8g2.sendBuffer();
}
...
void drawTime() {
  ...
  if (!getLocalTime(&timeinfo)) {
    Serial.println("Failed to get local time, should not be here?[404]");
  } else {
...
void drawSleepText() {
  u8g2.clearBuffer();
  u8g2.drawStr(10, 30, "Deepsleep time...");
  u8g2.sendBuffer();
}
